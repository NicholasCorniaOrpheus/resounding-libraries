	...
	for record in cat_dict:
		...
		if len(filter_query) >0 :
			for field in filter_query:
				...
				filter_subfields = list(filter(lambda x: filter_field[1] in x.keys(), subfields) )
				if len(filter_subfields) > 0: # filter subfield present
					# I assume only one subfield per field.
					if criterium in filter_subfields[0][filter_field[1]]:
...
def update_leader_from_catalogue_dict(cat_dict,filter_field,criterium,leader_position,value):
	...
	for record in cat_dict:
		...
		if False not in meets_criterium:
			...
			# get leader value
			leader = record["record"]["leader"]
			# string position is not supported in Python, convert to list
			list_leader = list(leader)
			if list_leader[leader_position] != " ": # I am ignoring wrongly formatted leaders...
				if list_leader[leader_position] != value: # Ignore if leader is already set to the right value
					new_list_leader = deepcopy(list_leader)
					new_list_leader[leader_position] = value
					new_leader = "".join(new_list_leader)
					print(f"Current record: {record["biblio_id"]}")
					#print(f"Control field: {record[filter_field[0]]}")
					print(f"Old leader: {leader}")
					print(f"New leader: {new_leader}")
					backup_records.append(record)
					# changed record
					changed_record = deepcopy(record)
					changed_record["record"]["leader"] = new_leader
					changed_records.append(changed_record)
			else:
				problematic_leaders.append({"biblio_id": record["biblio_id"],"leader": leader})
